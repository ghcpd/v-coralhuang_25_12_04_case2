...
class SQLiteStorage:
    ...
    def add_tasks_bulk(self, items: List[tuple]) -> None:
        """Add many tasks in a single transaction for performance.

        items: sequence of (task_text, tags_list)
        This will batch-insert rows and update tag usage and cooccurrence counts
        in aggregated form to avoid many commits.
        """
        if not items:
            return
        now = time.time()
        with self._lock:
            ...
            cur.executemany(
                "INSERT INTO tasks (task, completed, tags, created_at, updated_at) VALUES (?, ?, ?, ?, ?)",
                [(task, 0, json.dumps(tags or []), now, now) for (task, tags) in items],
            )

            # compute aggregate tag usage and cooccurrence
            usage_delta = {}
            co_delta = {}
            for task, tags in items:
                tags = sorted(set(tags or []))
                for t in tags:
                    usage_delta[t] = usage_delta.get(t, 0) + 1
                for i in range(len(tags)):
                    for j in range(i + 1, len(tags)):
                        pair = (tags[i], tags[j])
                        co_delta[pair] = co_delta.get(pair, 0) + 1

            # update tags usage_count in one go
            for t, delta in usage_delta.items():
                cur.execute(
                    "INSERT INTO tags (name, aliases, created_at, updated_at, usage_count) VALUES (?, '[]', ?, ?, ?) "
                    "ON CONFLICT(name) DO UPDATE SET usage_count = usage_count + ?, updated_at = ?",
                    (t, now, now, delta, delta, now),
                )

            # update cooccurrence in one go
            for (a, b), delta in co_delta.items():
                cur.execute(
                    "INSERT INTO tag_cooccurrence (a,b,weight) VALUES (?, ?, ?) "
